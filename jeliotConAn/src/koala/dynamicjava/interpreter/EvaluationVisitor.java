...
import jeliot.conan.ConflictiveForUpdate;
        ...
        try {
            ...
            try {
                ...
                while (cond == null
                	...
                	//We check if there is the possibility of creating a conflict by changing the 
                	//order of execution and updating before executing the body of the loop
                	//If there is another for loop conflict we also recreate the conflict
                	if(update!=null && !update.isEmpty() && (!ConAnUtilities.isReview()) && 
                			(conflict==null || conflict.getType().equals(Conflict.FOR))){
                		               		
                		
                		// Interpret the update statements
                        Iterator it = update.iterator();
                        //We create a new conflict for every iteration of the loop
                        conflict = new ConflictiveForUpdate(((Node)update.get(0)).getBeginLine());
                        ConAnUtilities.setConflict(conflict);
                        
                        MCodeUtilities.write(""+ Code.CONAN + Code.DELIM + conflict.getType());	
                        while (it.hasNext()) {
                        	((Node) it.next()).acceptVisitor(this);
                        }
                        
                		MCodeUtilities.write(""+ Code.CONAN_END + Code.DELIM + conflict.getType());
                    ...
                    // Interpret the update statements if there has been no conflict (normally)
                    if (update != null && conflict==null) {
                        Iterator it = update.iterator();
                        while (it.hasNext()) {
                            ((Node) it.next()).acceptVisitor(this);
                        }
    ...
    public Object visit(ObjectMethodCall node) {
        ...
        if ((node.hasProperty(NodeProperties.METHOD))
            ...
            if (node.hasProperty(NodeProperties.METHOD)) {
            	...
            	if (conflict != null && conflict.isOverridingConflict() &&
            			((ConflictiveOverriding)conflict).checkOverridenMethod(obj.getClass().getName(), m.getName())){
            		conflict.setConflictLine(node.getBeginLine());             
            	}
                // Fill the arguments
                if (larg != null) {
