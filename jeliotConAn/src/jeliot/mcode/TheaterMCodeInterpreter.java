...
public class TheaterMCodeInterpreter extends MCodeInterpreter {
	...
	protected void handleCodeCONAN(String type) {
		//ConAnUtilities.activateConflict(type);
		if (type.equals(Conflict.IMPLICIT_SUPER)){
			setInConAnSkipMCode();// = true;
		} else 
			//Number high to allow students to detect the conan while the alternative execution 
			// goes on . It will still give 3 more steps afterwards in CONAN_END
			ConAnUtilities.setAnswerCounter(300);
			//ConAnUtilities.activateConflict(type);
			
	}

	private void setInConAnSkipMCode() {
		this.inConAnSkipMCode++;
		
	}

	private void unsetIinConAnSkipMCode() {
		this.inConAnSkipMCode--;
		
	}
	protected void handleCodeCONAN_END(String string) {
		//director.resumeAnimation();
		unsetIinConAnSkipMCode();//inConAnSkipMCode = false;
