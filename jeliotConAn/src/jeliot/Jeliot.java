...
public class Jeliot {
    ...
    public void rewind() {
        ...
        //if we are in conan set av interaction engine
        AVInteractionEngine avinteractionEngine;
		try {
			avinteractionEngine = AVInteractionEngine.createAVInteractionEngine(
			        this.gui.getFrame(), userModel);
	        ConAnUtilities.setAvInteractionEngine(avinteractionEngine);
		} catch (Exception e) {
			e.printStackTrace();
			 if (DebugUtil.DEBUGGING) {
    ...
    protected AVInteractionMCodeInterpreter initializeInteractionEngine(TheaterMCodeInterpreter mCodeInterpreterForTheater) {
        try {
            ...
            AVInteractionEngine avinteractionEngine = AVInteractionEngine.createAVInteractionEngine(
                    this.gui.getFrame(), userModel);
