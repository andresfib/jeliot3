...
import jeliot.conan.ConflictCheck;
...
public class JeliotWindow implements PauseListener, MouseListener {
    ...
    private ActionListener conflictAction = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            Tracker.trackEvent(TrackerClock.currentTimeMillis(), Tracker.OTHER,
                    -1, -1, "ConflictChecked");
            if (ConAnUtilities.tooManyChecks()){
            	rewindAnimation();
            }
        }
    };
    ...
    public void tryToEnterAnimate(String methodCall) {
        ...
        if(!ConAnUtilities.isReview()){
        	 JOptionPane.showMessageDialog(JeliotWindow.this.getTheaterPane()
             		,  messageBundle.getString("conflict.message.find_conflict.content"),
             		messageBundle.getString("conflict.message.find_conflict.title"),          		
        ...
        if (ConAnUtilities.getConflict()!=null){
        	//JeliotWindow.this.director.getTheatre().requestFocus();
        	JOptionPane.showMessageDialog(JeliotWindow.this.getTheaterPane()
             		,  messageBundle.getString("conflict.message.find_conflict.content"),
             		messageBundle.getString("conflict.message.find_conflict.title"),          		
                    JOptionPane.INFORMATION_MESSAGE);
    ...
    void checkConflict() {
    	...
    	if (result){
            ...
            this.runUntilLine = c.getConflictLine();
            //msg = "Right! You found the conflictive animation";
            ConflictCheck.check();
            //We get Jeliot interface back to normal,ie, play button back
            conPan.swapConflictiveButton();
            ConAnUtilities.setReview(true);
       	 JOptionPane.showMessageDialog(JeliotWindow.this.getTheaterPane()
          		,  messageBundle.getString("conflict.message.found_conflict.content"),
          		messageBundle.getString("conflict.message.found_conflict.title"),          		
                 JOptionPane.INFORMATION_MESSAGE);
     
            rewindAnimation();            
    	} else {
    		msg = ConAnUtilities.getMessageWrongAnswer();
    	    JOptionPane.showMessageDialog(JeliotWindow.this.getTheaterPane()
    ...
    public void runUntil(int lineNumber) {
        if (lineNumber > 0) {
    ...
    class ControlPanel extends JPanel{
	    ...
	    public void swapConflictiveButton(){
	    	bp.removeAll();
	    	//bp.setLayout(new GridLayout(1, 4));
	    	bp.add(stepButton);
//	    	playButton = makeControlButton(messageBundle.getString("button.play"),
