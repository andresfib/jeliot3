...
<classes info> ::= <class info> | <classes info> <class info>
<class info> ::= \p{CLASS METHOD CONSTRUCTOR FIELD}\?  \p{END_CLASS}
...
<parameters info> ::= <parameter info> | <parameters info> <parameter info>
<parameter info> ::= \p{BEGIN} ??????????? \p{P}

<method body> ::= <block statements>
<block statements> ::= <block statement> | <block statements><block statement>
<block statement> ::= <variable declaration> | <statement>
<scoped block> ::= \p{SCOPE 1} <block statements> \p{SCOPE 0}

<statement> ::= <simple statement> | <if statement> | <for statement> | <while statement> 
<simple statement> ::= <scoped block> | <statement expression> | <switch statement> | <do statement> | <continue statement> | <break statement> | <return statement>
<statement expression> ::= <assignment> | <pre expression> | <post expression> | <method call> | <class allocation>

<if statement> ::= <expression> \p{IFT} <statement>? |  <expression> \p{IFTE} <statement>? 
<while statement> ::= <expression> \p{WHI} <statement>? | <expression> \p{WHI} <statement>
<switch statement> ::= \p{SWITCHB} <expression> \p{SWITCHBF} <block>? SWITCH
<for statement> ::= ..........
...
<expression> ::= <assignment> | <binary expression> | <unary expression> | <primary>

<assignments> ::=  \p{BEGIN} <expression> \p{TO} <left hand side>
<left hand side> ::= <identifier> | <field access> | <array access>
...
<primary> ::= <literal> | <field access> | <array access> | <class allocation> | <array allocation> | <method call>

<literal> ::= \p{L}
<field access> ::= <primary> <identifier> OFA
<array access> ::= <identifier> <dim expressions> AAC
<dim expressions> ::= <expression> | <dim expressions> <expression>
<method call> ::= <static method call> | <object method call>
