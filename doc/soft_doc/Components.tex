...
\subsection{Jeliot Class}
\label{sec:Jeliot_Class}

\subsection{User Interface}
\label{sec:User_Interface}

The user interface of Jeliot is located in the package \p{jeliot.gui}. The main
class is \p{JeliotWindow} that extends \p{JFrame}. The frame is layed out
by \p{BorderLayout}. A split pane is added in the center and a panel containing
\p{Control panel} (created in \p{JeliotWindow}) and \p{Output console}
(\p{OutputConsole}) is added in the south border. In the split pane the
left side is used by \p{Code editor} (\p{CodeEditor}) or \p{Code viewer}
(\p{CodeEditor}). \p{Code editor} is shown during editing of the program
and \p{Code viewer} during animation of the program. On the right hand side
of the split pane a \p{Theatre} is normally shown. If any errors are
found durint the compilation or running of the program they are shown in
the \p{Error viewer} (\p{ErrorViewer}). The structure of the user interface
...
the appropriate class (e.g. \p{Jeliot})
...
Other classes in the \p{gui} package are related to one of these components.
...
to each other and to the \p{Actor}s.}
