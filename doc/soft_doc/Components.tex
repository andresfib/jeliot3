...
\p{ConstructorInvocation}, \p{Object\-Field\-Access},
...
contained. The interface \p{ExpressionStatement} is used only when
\djava{} parses the source code to build up the \p{ForStatement}
node. \p{ExpressionContainer} (\p{ObjectMethodCall},
...
implementing the \p{ContinueTarget} interface in order to allow
\p{ContinueStatements} inside of them.
