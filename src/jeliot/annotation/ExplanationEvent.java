...
public class ExplanationEvent {
	...
	//use "show" to control JScrollPanel 
	...
	public void explanationDisplay(String name,String explanation){
		...
		//set scroller final because it will be used in a inner class
		...
		final JButton button = new JButton(messageBundle.getString("message.button1"));
		//mix the first and second part,another way is:InfoPanel mix = new InfoPanel(content,scroller);
		...
		ActionListener al = new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) { 
				...
				if(show)
				{
					scroller.setVisible(!show);
					button.setText(messageBundle.getString("message.button2"));
					Container ancestor = jpanel.getTopLevelAncestor( ); 
		...
		//add(content) rather than add(pane) because the latter one keep the JOptionpane moved 
		jpanel.add(content);
		jpanel.add(button);
