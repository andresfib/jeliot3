...
public class JeliotWindow {
	...
	/**
	 * The version information about Jeliot from name and version
	 * from the resource bundle. 
	 */
	private String jeliotVersion =
		bundle.getString("name") + bundle.getString("version");

	/**
	 * Should the messages during the program visualization 
	 * be shown as message dialogs.
	 */
	private boolean showMessagesInDialogs = false;

	/**
	 * True if an error has occured during the execution and
	 * false if no error was encountered. 
	 */
	...
	/** Menu items that should be either enabled or disabled when the animation mode
	 * is entered or exited respectively.
	 */
			...
			if (editor.isChanged()) {
				int n =
					JOptionPane.showConfirmDialog(
						frame,
						bundle.getString("quit.without.saving.message"),
						bundle.getString("quit.without.saving.title"),
	...
	/**
	 * Adds the given JMenu's JMenuItems into the Vector
	 * animationMenuItems.
	 */
	...
	private JMenu makeAnimationMenu() {
		...
		final JCheckBoxMenuItem cbmenuItem =
			new JCheckBoxMenuItem(
				bundle.getString("menu.animation.pause_on_message"),
				showMessagesInDialogs);
		cbmenuItem.setMnemonic(KeyEvent.VK_D);
		cbmenuItem.setAccelerator(
	...
	private JPanel makeControlPanel() {
		...
		editButton =
			makeControlButton(
				bundle.getString("button.edit"),
				bundle.getString("image.edit_icon"));
		compileButton =
			makeControlButton(
				bundle.getString("button.compile"),
		...
		stepButton =
			makeControlButton(
				bundle.getString("button.step"),
				bundle.getString("image.step_icon"));
		stepButton.setMnemonic(KeyEvent.VK_S);
		playButton =
			makeControlButton(
				bundle.getString("button.play"),
				bundle.getString("image.play_icon"));
		playButton.setMnemonic(KeyEvent.VK_P);
		pauseButton =
			makeControlButton(
				bundle.getString("button.pause"),
				bundle.getString("image.pause_icon"));
		pauseButton.setMnemonic(KeyEvent.VK_U);
		rewindButton =
			makeControlButton(
				bundle.getString("button.rewind"),
		...
		JLabel jicon =
			new JLabel(
				new ImageIcon(
					bundle.getString("directory.images")
		...
		try {
			try {
				...
				if (methodCall != null) {
					...
					showErrorMessage(
						bundle.getString("main_method_not_found.exception"));
	...
	*/
    ...
    /**
     * Shows the given error message and sets the buttons
     * and menuitems for animation as disabled.
     * @param e the error message in String
     */
	public void showErrorMessage(String e) {
		...
		String[] s2 =
			{
				bundle.getString("menu.animation.step"),
     ...
     * @param e interpreter error that contains the error message and the highlighting information.
    ...
    /**
     * Sets the given menu items contained in the second parameter
     * either enabled or disabled depending the value of the first parameter 
     * @param enabled if true means that the given menu items should be enabled if false the menu items should be disabled.
     * @param menuItems the menu items to be enabled or disabled.
     */
	 ...
	 * Calls methods jeliot.rewind() and theatre.repaint().
	 *
	...
	public void runUntil() {
		String inputValue =
			JOptionPane.showInputDialog(
				bundle.getString("dialog.run_until"),
				new Integer(0));
		int lineNumber = 0;
